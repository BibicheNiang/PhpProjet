...
<div id="contenu">
    <div class="row" style="max-height: 50rem;"> 
        <div class="col-sm-7">
            <div class="card" >
                <div class="card-header bg-transparent border-success">Liste des factures</div>
                    <div class="card-body">
                        <table class= "table table-responsive table-striped">
                          ...
                          @foreach($factures as $facture)
                            <tr>
                                <td>{{$facture->id}}</td>
                                <td>{{$facture->mois}}</td>
                                <td>{{$facture->comsommation}}</td>
                                <td>{{$facture->prix}}</td>
                                <td>{{$facture->reglement}}</td>
                                <td>{{$facture->abonnement_id}}</td>
                                <td><a href='/editedd/{{$facture->id}}'>Editer</a></td>
                                <td><a href='/destroye/{{$facture->id}}'>Supprimer</a></td>
                                <td><a href='/generer/{{$facture->id}}'>Generer PDF</a></td>
                            </tr>
                            @endforeach
                        </table>
                    </div>
        ...
        <div class="col-sm-5">
            <div class="card" style="max-height: 100rem;">
                <div class="card-header bg-transparent border-success">Formulaire des factures</div>
                    <div class="card-body">
                        <form action="/store" method="POST">
                        {{ csrf_field() }}
                            <div class="form-group">
                                <label class="control-label" for="nom">mois</label>
                                <input type="text" require type="mois" name="nom" class="form-control" id="mois" >
                            </div>
                            <div class="form-group">
                                <label class="control-label" for="abonnement">Abonnement</label>
                                <Select name="abonnement"  class="form-control" id="abonnement">
                                  @foreach($abonnements as $liste)
                                  <option value="{{$liste->id}}"> {{$liste->contrat}}</option>
                                  @endforeach
                                </Select>
                            </div>
                            <div class="form-group">
                                <input type="submit" class="btn btn-success" name="ajouter" id="ajouter" value="Generer Facture"/>
